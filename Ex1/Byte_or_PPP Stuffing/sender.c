...
void byte_stuff(const char *in_data, char *out_data)
{
    int i, j = 0;
    int len = strlen(in_data);
    char current_byte[9];

    // Read payload string 8 bits at a time
    for (i = 0; i < len; i += 8)
    {
        strncpy(current_byte, in_data + i, 8);
        current_byte[8] = '\0';

        // Check if matching FLAG (01111110) or ESC (01111101)
        if (strcmp(current_byte, "01111110") == 0 || strcmp(current_byte, "01111101") == 0)
        {
            // Inject ESC byte
            strcpy(out_data + j, "01111101");
            j += 8;
        }
        
        // Append original byte contents
        strcpy(out_data + j, current_byte);
        j += 8;
    }
    out_data[j] = '\0';
}
...
int main()
{
    ...
    printf("--- PART 5B: BYTE STUFFING & TRANSMISSION LAYER ---\n");
    FILE *t_file = fopen("transmission.txt", "w");
    if (!t_file)
    {
        printf("ERROR! Could not create transmission.txt file.\n");
        return 1;
    }
    ...
    for (i = 0; i < total_frames; i++)
    {
        char stuffed_data[100] = {0}; // Increased array size to accommodate potential escape additions
        byte_stuff(frames[i].data, stuffed_data);

        char final_frame[500] = {0};
        strcpy(final_frame, flag);             // Beginning Flag
        strcat(final_frame, src_mac_bin);      // Source MAC Binary
        strcat(final_frame, dest_mac_bin);     // Destination MAC Binary
        strcat(final_frame, flag);             // Data Start Flag
        strcat(final_frame, stuffed_data);     // Byte Stuffed Data
        strcat(final_frame, flag);             // Data End Flag
        strcat(final_frame, frames[i].trailer); // Trailer
        strcat(final_frame, flag);             // Ending Flag

        printf("[Frame %d] Original Data : %s\n", frames[i].frame_no, frames[i].data);
        printf("[Frame %d] Stuffed Data  : %s\n", frames[i].frame_no, stuffed_data);
        printf("[Frame %d] Final Frame  : %s\n\n", frames[i].frame_no, final_frame);

        fprintf(t_file, "%s\n", final_frame);
    }
    fclose(t_file);
    printf("Transmitted bits written to transmission.txt successfully.\n");
    printf("--------------------------------------------------------------------\n\n");
